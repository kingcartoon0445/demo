...
export default function EditChatbotDialog({
    ...
    const [fbSelectedList, setFbSelectedList] = useState(fbList);
    const [zlSelectedList, setZlSelectedList] = useState(zlList);
    const [replyType, setReplyType] = useState(dReplyType);
    const [chatBotType, setChatBotType] = useState(dChatbotType);
    const [prompt, setPrompt] = useState(dPrompt);
    const [isLoading, setIsLoading] = useState(false);
    const [openFbFeedList, setOpenFbFeedList] = useState(false);
    const [fbFeedSelectedList, setFbFeedSelectedList] = useState(fbFeedList);
        ...
        toast.promise(
            editChatBot(orgId, trainId, {
                ...
                .then((res) => {
                    ...
                    toast.success("Chỉnh sửa kịch bản thành công");
                    isDone();
                    setOpen(false);
                })
                .catch((e) => {
    ...
    const handleDelete = async () => {
        setIsDeleting(true);
        try {
            const res = await deleteChatBot(orgId, trainId);
            if (res?.code !== 0 || res?.message) {
                setIsDeleting(false);
                return; // API đã tự hiển thị toast.error
            }
            toast.success("Xóa kịch bản thành công");
            setShowDeleteDialog(false);
            setOpen(false);
            isDone(); // Reload list chatbot
        } catch (error) {
            console.error("Error deleting chatbot:", error);
            toast.error("Đã có lỗi xảy ra khi xóa kịch bản");
            setIsDeleting(false);
        }
    };
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="min-w-[1200px] min-h-[80vh] flex gap-4 overflow-hidden">
                    ...
                    <div className="flex gap-4">
                        ...
                        <div className="flex flex-col flex-1">
                            ...
                            <Textarea
                                ...
                                className="bg-[var(--bg1)] mt-2 h-[390px] overflow-y-auto resize-none"
                            />
                        </div>
                    </div>
            ...
            <CustomerAlertDialog
                ...
                title="Xóa kịch bản"
                subtitle="Bạn có chắc chắn muốn xóa kịch bản này? Hành động này không thể hoàn tác."

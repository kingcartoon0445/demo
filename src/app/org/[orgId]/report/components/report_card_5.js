...
export default function ReportCard5({ reportData, isLoading }) {
    ...
    const processedData = useMemo(() => {
        ...
        staffArray.forEach((staff) => {
            const otherCustomers =
                staff.total - staff.transaction - staff.potential;
    ...
    const displayedData = showDetailButton
        ? processedData.slice(0, displayLimit)
        : processedData;

    return (
        <Card className="w-full h-full overflow-hidden min-h-[300px] border-none shadow-none !bg-transparent">
            {detailOpen && (
                <ReportCard5Dialog
                    open={detailOpen}
